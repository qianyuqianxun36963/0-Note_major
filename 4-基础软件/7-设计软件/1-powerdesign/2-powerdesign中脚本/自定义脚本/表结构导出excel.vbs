...
Sub ShowTable(tab, sheet)  
   If IsObject(tab) Then  
     ...
     rowsNum = rowsNum + 1  
      ...
      sheet.cells(rowsNum, 5) = tab.comment  
...
colsNum = 0  
      ...
      sheet.cells(rowsNum, 2) = col.comment  
        sheet.cells(rowsNum, 3) = ""  
      Dim point 
      point = InStrRev (col.comment,",") + InStrRev (col.comment,"，") + InStrRev (col.comment," ") + InStrRev (col.comment,":") + InStrRev (col.comment,"：")
      if point > 0 then
        sheet.cells(rowsNum, 4) = left(col.comment,point-1)
      else
